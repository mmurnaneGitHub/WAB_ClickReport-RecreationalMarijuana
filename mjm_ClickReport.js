...
define([
        ...
        //Query layer - MJ Zoning  
        var qtCOT = new QueryTask("https://gis.cityoftacoma.org/arcgis/rest/services/PDS/DARTzoning/MapServer/17");  //MJ Zoning 
        var qCOT = new Query();
        ...
        handleQueryParcel: function(results) {
          ...
          parcel = results.features;
            ...
            if (parcel.length>0) {
              ...
              var title = "Parcel " + currentParcel;
              address = "<div><b>Address:</b> " + parcel[0].attributes["Site_Address"]  +  "</div>" +
                        "<div style='clear:both;float:right;'><a title='Assessor Information Link' href=\"http://epip.co.pierce.wa.us/CFApps/atr/epip/summary.cfm?parcel=" + parcel[0].attributes["TaxParcelNumber"]  + "\" target=\"_blank\">" +
                        "Assessor</a>&nbsp;<br>&nbsp;</div>" +
                        ...
                        qtCOT.execute(qCOT, function(results) {  
                          ...
                          var COT_Message = "";
                              ...
                              if (results.features.length>0) {  
				                  var allowedUses = "<div style='float:left;'><br><b>Compliant with I-502: </b></div>";
                    				arrayUtils.forEach(results.features, function(resultsRec) { //loop through multiple records
					                  if (resultsRec.attributes["Allowed"] == 'Retail') {
					                      allowedUses += "<div style='clear:both;float:right;'><i>Retail Only</i>&nbsp;<br>&nbsp;</div>";
					                  } else if (resultsRec.attributes["Allowed"] == 'Production, Processing') {
					                      allowedUses += "<div style='clear:both;float:right;'><i>Production & Processing Only</i>&nbsp;<br>&nbsp;</div>";
					                  } else if (resultsRec.attributes["Allowed"] == 'Retail, Production, Processing') {
					                      allowedUses += "<div style='clear:both;float:right;'><i>Production, Processing & Retail</i>&nbsp;<br>&nbsp;</div>";
					                  } else {
					                      //just in case an extra 'zoning' slipped in
					                      allowedUses += resultsRec.attributes["Allowed"];
					                  }
                    				});

                                  COT_Message += allowedUses;
                              } else {
                                  COT_Message += "<div style='clear:both;'></div><div><br><b>Not Compliant with I-502:</b><br>This site is not suitable for I-502 related uses, including the production, processing, or retail sale of recreational marijuana.<br>&nbsp;</div>";
                              }
                          ...
                          alert("Error in Marijuana Zoning identify: " + err.message);
                          console.error("Identify Marijuana Zoning Error: " + err.message);
                        });
